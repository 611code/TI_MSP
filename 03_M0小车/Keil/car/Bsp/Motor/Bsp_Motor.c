...
void load_motor_pwm(int motor1_pwm,int motor2_pwm)
{
    ...
    //2.装载PWM值
    // __HAL_TIM_SET_COMPARE(&Motor_tim,TIM_CHANNEL_1,find_abs(motor2_pwm));   // 设置比较寄存器的值 
    DL_TimerG_setCaptureCompareValue(PWM_MOTOR_L_INST,find_abs(motor2_pwm),GPIO_PWM_MOTOR_L_C1_IDX);
    ...
    //2.装载PWM值
    // __HAL_TIM_SET_COMPARE(&Motor_tim,TIM_CHANNEL_4,find_abs(motor1_pwm));   // 设置比较寄存器的值 
    DL_TimerG_setCaptureCompareValue(PWM_MOTOR_R_INST,find_abs(motor1_pwm),GPIO_PWM_MOTOR_R_C1_IDX);
}
...
void set_motor1_disable(void)
{
    ...
    DL_TimerG_setCaptureCompareValue(PWM_MOTOR_R_INST,0,GPIO_PWM_MOTOR_R_C1_IDX);
}
...
void set_motor2_disable(void)
{
    ...
    DL_TimerG_setCaptureCompareValue(PWM_MOTOR_L_INST,0,GPIO_PWM_MOTOR_L_C1_IDX);
}
